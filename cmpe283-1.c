...
/*
 * VMX configuration for 20 output instructions.
 */
...
struct capability_info procbasedsecondary[23] =
{
...
detect_vmx_features(void)
{
	...
	//MSRs based Processor implementation
	...
	pr_info("Secondary Processor based: 0x%llx\n",
		(uint64_t)(lo | (uint64_t)hi << 32));
	report_capability(procbasedsecondary, 23, lo, hi);
